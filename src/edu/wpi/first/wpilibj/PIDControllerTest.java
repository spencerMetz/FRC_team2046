 ...
 */
public class PIDControllerTest implements PIDInterface, LiveWindowSendable, Controller {
  ...
  private class PIDTask extends TimerTask {
    ...
    private PIDControllerTest m_controller;

    public PIDTask(PIDControllerTest controller) {
      if (controller == null) {
   ...
   *        default is 50ms.
   */
  public PIDControllerTest(double Kp, double Ki, double Kd, double Kf, PIDSource source,
   ...
   *        default is 50ms.
   */
  public PIDControllerTest(double Kp, double Ki, double Kd, PIDSource source, PIDOutput output,
      double period) {
...
//    return getSetpoint() - m_pidInput.pidGet();
  ...
  public boolean isEntirelyOnTarget(){
  	return isAvgErrorValid() && bufferGood();
  }
  private boolean bufferGood(){
  	boolean result = true;
  	for(Double e:m_buf){
  		if(Math.abs(e)>=((AbsoluteTolerance)m_tolerance).value){
  			result = false;
  		}
  	}
  	return result;
